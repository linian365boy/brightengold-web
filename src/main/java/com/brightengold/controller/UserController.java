...
public class UserController {
	...
	public MessageVo add(User user, HttpServletRequest request) {
		...
		try {
			...
			//设置用户选择的权限；设置默认权限
			List<Role> roles = Arrays.asList(role,defaultR);
			user.setRoles(roles);
	...
	public MessageVo update(HttpServletRequest request,@PathVariable Integer userId,User user) {
		...
		//判断是否修改角色
		if(StringUtils.isBlank(role) && CollectionUtils.isEmpty(temp.getRoles())){
			//没有修改角色
